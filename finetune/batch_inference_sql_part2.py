"""
批量推理脚本 - 第二批：从2025-12-12往前回溯100天

优化：
- batch size 128（GPU显存有余量）
- 增量保存（边推理边写入，避免崩溃丢失）
"""
...
class BatchPredictor:
    ...
    def predict_batch(self, data_dict, lookback=400, pred_len=10, end_indices=None, T=1.0, top_p=0.9):
        ...
        B = len(data_list)
        batch_size = min(B, 128)  # 增大batch size
        predictions = {}
...
def main():
    ...
    # 第二批起点：上次结束位置 2025-12-12
    start_from_date = '2025-12-12'
    ...
    print(f"\n第二批预测范围:")
    print(f"  起点: {start_date.strftime('%Y-%m-%d')} (上次结束)")
    print(f"  终点: {end_date.strftime('%Y-%m-%d')}")
    print(f"  倒推天数: {max_days}")

    # 增量保存：打开文件准备写入
    output_path = 'outputs/prediction_results/predictions_mini_part2.sql'
    created_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f+08')

    # 打开文件，增量写入
    output_file = open(output_path, 'w', encoding='utf-8')

    print(f"\n开始批量预测 (batch size=128)...")
