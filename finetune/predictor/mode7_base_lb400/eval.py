"""
Mode7 模型评估 - Kronos-base + MA60 lb400_pd10

用法:
    python eval.py --model outputs/models/mode7_base_lb400_ddp/checkpoints/best_ic_model
"""
...
def evaluate_trajectory_ic(model, tokenizer, device, all_data, test_indices,
                           n_samples=500, lookback=400, pred_len=10, clip=5.0, rng=None):
    """Trajectory IC 测试"""
...
def main():
    parser = argparse.ArgumentParser(description='Mode7 Model Evaluation')
    parser.add_argument('--model', type=str,
                        default='outputs/models/mode7_base_lb400_ddp/checkpoints/best_ic_model')
    parser.add_argument('--tokenizer', type=str,
                        default='outputs/tokenizers/ma60_tokenizer_base_v1/checkpoints/best_model')
    parser.add_argument('--data-dir', type=str, default='finetune/data/ma60_norm/windowed_lb400_pd10')
    ...
    print("=" * 60)
    print("Mode7 Model Evaluation (Kronos-base + MA60 lb400)")
    print("=" * 60)
    ...
    # Model - 使用 Kronos-base
    model = Kronos.from_pretrained(os.path.join(project_root, "pretrained/Kronos-base"))
    ...
    print_evaluation_result(result, predict=args.predict, title="Mode7 Test Set Evaluation")
