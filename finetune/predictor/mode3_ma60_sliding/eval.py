"""
Mode3 模型评估 - MA60 sliding 归一化
使用测试集计算 Trajectory IC / MAE / DA

用法:
    python eval.py --model outputs/models/mode3_lb60_pd10/checkpoints/best_ic_model
"""
...
def evaluate_trajectory_ic(model, tokenizer, device, all_data, test_indices,
                           n_samples=500, lookback=60, pred_len=10, clip=5.0, rng=None):
    """Trajectory IC 测试"""
...
def main():
    parser = argparse.ArgumentParser(description='Mode3 Model Evaluation')
    parser.add_argument('--model', type=str,
                        default='outputs/models/archived/mode3_lb60_pd10/checkpoints/best_ic_model')
    parser.add_argument('--tokenizer', type=str,
                        default='outputs/tokenizers/ma60_tokenizer_base_v1/checkpoints/best_model')
    parser.add_argument('--data-dir', type=str, default='finetune/data/ma60_norm/windowed_lb60_pd10')
    ...
    print("=" * 60)
    print("Mode3 Model Evaluation (MA60 sliding)")
    print("=" * 60)
    ...
    # 打印结果
    print_evaluation_result(result, predict=args.predict, title="Mode3 Test Set Evaluation")
