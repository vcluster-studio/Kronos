"""
Mode4 模型评估 - MA60 weighted 归一化
使用测试集计算 Trajectory IC / MAE / DA

用法:
    python eval.py --model outputs/models/mode4_lb60_pd5/checkpoints/best_ic_model
"""
...
def evaluate_trajectory_ic(model, tokenizer, device, all_data, test_indices,
                           n_samples=500, lookback=60, pred_len=5, clip=5.0, rng=None):
    """Trajectory IC 测试"""
...
def print_evaluation_result(result, predict, title="Evaluation Results"):
    ...
    print("\nCore Bucket (2%~5%) 指标:")
    bucket_2_5 = '2%~5%'
    n_2_5 = result.get(f'bucket_{bucket_2_5}_n', 0)
    ic_2_5 = result.get(f'bucket_{bucket_2_5}_ic')
    da_2_5 = result.get(f'bucket_{bucket_2_5}_da')
    traj_ic_2_5 = result.get(f'bucket_{bucket_2_5}_trajectory_ic')
    ...
    print(f"  N={n_2_5}, IC={ic_str}, DA={da_str}, TrajIC={traj_str}")

    print("=" * 60)


def main():
    parser = argparse.ArgumentParser(description='Mode4 Model Evaluation')
    parser.add_argument('--model', type=str,
                        default='outputs/models/mode4_lb60_pd5/checkpoints/best_ic_model')
    parser.add_argument('--tokenizer', type=str,
                        default='outputs/tokenizers/ma60_tokenizer_base_v1/checkpoints/best_model')
    parser.add_argument('--data-dir', type=str, default='finetune/data/ma60_norm/windowed_lb60_pd5')
    ...
    print("=" * 60)
    print("Mode4 Model Evaluation (MA60 weighted)")
    print("=" * 60)
    ...
    # 打印结果
    print_evaluation_result(result, predict=args.predict, title="Mode4 Test Set Evaluation")
