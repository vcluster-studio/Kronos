"""
MA60 Predictor Training Script - Mode4 (lookback=60, predict=5, weighted loss)

使用 MA60 滑动归一化数据训练 predictor。
短 lookback (60步) + predict=5 + 样本权重 + delta loss

关键配置：
- MA60 归一化数据
- lookback=60 (短窗口)
- predict=5
- 样本权重: w = clip(abs_return / 0.02, 0.2, 2.0)
- delta loss 只对 |return| > 2% 生效

Usage:
    python -u finetune/predictor/mode4_ma60_weighted/train.py
"""
...
# MA60 lb60_pd5 预归一化数据路径
DATA_PATHS = {
    'train': 'finetune/data/ma60_norm/windowed_lb60_pd5/train_data.pkl',
    'val': 'finetune/data/ma60_norm/windowed_lb60_pd5/val_data.pkl',
    'test': 'finetune/data/ma60_norm/windowed_lb60_pd5/test_data.pkl',
}
...
TRAINING_PARAMS = {
    ...
    'ma_window': 60,
    'lambda1': 0.05,  # delta loss weight
}
...
def compute_sample_weight(abs_return):
    """样本权重函数"""
    base_weight = np.clip(abs_return / 0.02, 0.2, 2.0)
    if abs_return > 0.05:
        return 0.5
    return base_weight


class MA60WeightedDataset(Dataset):
    """使用预归一化的 MA60 数据 + 样本权重"""
    ...
    def __getitem__(self, idx):
        ...
        orig = data['original'][start_idx:end_idx].astype(np.float32)
        ...
        # 计算样本权重 (基于 t+1 的 actual_return)
        lb = self.config.lookback
        baseline_close = orig[lb - 1, 3]
        actual_close_t1 = orig[lb, 3]
        actual_return = abs((actual_close_t1 - baseline_close) / (abs(baseline_close) + 1e-8))
        sample_weight = compute_sample_weight(actual_return)
        delta_valid = actual_return > 0.02

        original_close_end = orig[lb + self.config.predict - 1, 3]
        direction = original_close_end > baseline_close
        ...
        return (torch.from_numpy(x_norm), torch.from_numpy(x_stamp),
                ...
                torch.tensor(int(delta_valid), dtype=torch.long))


def quick_trajectory_ic_test(model, tokenizer, device, val_data, n_samples=500,
                              lookback=60, pred_len=5, clip=5.0, rng=None):
    """Trajectory IC 测试"""
    ...
    bucket_trajectories = {k: [] for k in BUCKETS.keys()}
    ...
    if has_windows:
        ...
        for idx in sample_indices:
            ...
            try:
                ...
                # 分桶
                baseline_close = orig_full[lookback - 1, 3]
...
def train_model(model, tokenizer, device, config, save_dir, val_data=None):
    ...
    print(f"lambda1 (delta loss): {config.lambda1}")
    ...
    train_dataset = MA60WeightedDataset('train', config=config)
    val_dataset = MA60WeightedDataset('val', config=config)
    ...
    # Horizon 衰减权重
    if config.predict > 1:
        h_weights = torch.tensor([config.horizon_gamma ** i for i in range(config.predict)])
    else:
        h_weights = torch.tensor([1.0])
    ...
    for epoch_idx in range(config.epochs):
        ...
        for i, (batch_x, batch_stamp, batch_direction, batch_weight, batch_delta_valid) in enumerate(train_loader):
            ...
            batch_delta_valid = batch_delta_valid.to(device, non_blocking=True)
            ...
            # CE loss with sample weights
            p = config.predict
            weights = h_weights.to(device)

            ce_loss = 0.0
            for step_i in range(p):
                w = weights[step_i]
                per_sample_ce = F.cross_entropy(s1_logits[:, -(p - step_i), :], token_out[0][:, -(p - step_i)], reduction='none')
                per_sample_ce = per_sample_ce * batch_weight
                ce_loss = ce_loss + w * per_sample_ce.mean()

                per_sample_ce = F.cross_entropy(s2_logits[:, -(p - step_i), :], token_out[1][:, -(p - step_i)], reduction='none')
                per_sample_ce = per_sample_ce * batch_weight
                ce_loss = ce_loss + w * per_sample_ce.mean()
            ce_loss = ce_loss / weights.sum()
        ...
        with torch.no_grad():
            for batch_x, batch_stamp, batch_direction, batch_weight, batch_delta_valid in val_loader:
                batch_x = batch_x.to(device, non_blocking=True)
                batch_stamp = batch_stamp.to(device, non_blocking=True)
                batch_direction = batch_direction.to(device, non_blocking=True)
                batch_weight = batch_weight.to(device, non_blocking=True)
        ...
        if traj_result:
            ...
            # Core bucket
            bucket_2_5 = '2%~5%'
            n_2_5 = traj_result.get(f'bucket_{bucket_2_5}_n', 0)
            ic_2_5 = traj_result.get(f'bucket_{bucket_2_5}_ic')
            da_2_5 = traj_result.get(f'bucket_{bucket_2_5}_da')
            traj_ic_2_5 = traj_result.get(f'bucket_{bucket_2_5}_trajectory_ic')

            ic_str = f"{ic_2_5:.4f}" if ic_2_5 is not None else "N/A"
            da_str = f"{da_2_5:.1%}" if da_2_5 is not None else "N/A"
            traj_str = f"{traj_ic_2_5:.4f}" if traj_ic_2_5 is not None else "N/A"
            print(f"Core Bucket (2%~5%): N={n_2_5}, IC={ic_str}, DA={da_str}, TrajIC={traj_str}")
...
def main():
    parser = argparse.ArgumentParser(description='Mode4 Training (lb60, pd5, weighted)')
    parser.add_argument('--epochs', type=int, default=30)
    parser.add_argument('--batch-size', type=int, default=16)
    parser.add_argument('--lr', type=float, default=0.003)
    parser.add_argument('--n-samples', type=int, default=500)
    parser.add_argument('--resume', type=str, default=None)
    parser.add_argument('--save-folder', type=str, default='mode4_lb60_pd5')
    ...
    print("\n" + "="*60)
    print("Mode4 Training completed!")
