"""
MA60 Predictor Training Script - Mode6 (lookback=200)

使用 MA60 滑动归一化数据训练 predictor。
中等 lookback (200步) 对比验证 lookback 影响。
...
Usage:
    python -u finetune/predictor/mode6_ma60_lb200/train.py
"""
...
# MA60 lb200_pd10 预归一化数据路径
DATA_PATHS = {
    'train': 'finetune/data/ma60_norm/windowed_lb200_pd10/train_data.pkl',
    'val': 'finetune/data/ma60_norm/windowed_lb200_pd10/val_data.pkl',
    'test': 'finetune/data/ma60_norm/windowed_lb200_pd10/test_data.pkl',
}
...
def quick_trajectory_ic_test(model, tokenizer, device, val_data, n_samples=500,
                              lookback=200, pred_len=10, clip=5.0, rng=None):
    """Trajectory IC 测试"""
...
def main():
    parser = argparse.ArgumentParser(description='MA60 Predictor Training (Mode6 lb200)')
    parser.add_argument('--epochs', type=int, default=30)
    parser.add_argument('--batch-size', type=int, default=16)
    parser.add_argument('--lr', type=float, default=0.003)
    parser.add_argument('--n-samples', type=int, default=500)
    parser.add_argument('--resume', type=str, default=None)
    parser.add_argument('--save-folder', type=str, default='mode6_lb200_pd10')
    args = parser.parse_args()

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    print("\n" + "="*60)
    print("MA60 Predictor Training (Mode6 lb200)")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("Mode6 MA60 lb200 Training completed!")
